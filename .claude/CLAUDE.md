...
A **project cookbook** (`cookbook.json`) assembles recipes and ingredients into a complete application, plugin, or widget. See `reference/cookbook.schema.json` for the JSON Schema. Distinguished from the top-level cookbook (this repo), which provides the source artifacts.
...
1. `git worktree add .claude/worktrees/<branch-name> -b <branch>`
...
6. Clean up: `git worktree remove .claude/worktrees/<branch-name>`
