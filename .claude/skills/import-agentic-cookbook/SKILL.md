...
**Directory layout**: This skill lives inside the agentic cookbook repo. Cookbook content (guidelines, recipes, principles) is at `cookbook/` from the repo root. The target repo to analyze must be provided as an argument or the user will be prompted for it.
...
5. Read this repo's CLAUDE.md and cookbook/conventions.md. Read the recipe template at cookbook/recipes/_template.md. Scan existing recipes in cookbook/recipes/ (recursively list all subdirectories: ui/component/, ui/panel/, ui/window/, infrastructure/, app/)
...
- The Roadmap's acceptance criteria become recipe REQ-NNN candidates
...
11. Find and read test files for this feature:
    ...
    ```
    Test names describe expected behavior → these become recipe conformance test vectors.
...
Synthesize findings into extractable recipe bundles. See `${CLAUDE_SKILL_DIR}/references/extraction-criteria.md` for extraction and bundling criteria.
...
5. **Infrastructure compliance**: Which cookbook guidelines are already met, which are missing
...
8. **Validate against PR history**: Does each bundle correspond to one or a few PRs? If so, the PR descriptions provide additional context for the recipe.
9. **Include complexity scores**: Higher complexity bundles are more valuable to extract as recipes (more decisions to preserve).
10. **Include test coverage**: Bundles with tests are better extraction candidates — the tests validate the recipe.

Compare each bundle against existing cookbook recipes:
- Read the existing recipe fully
- Read the repo's implementation fully
- Side-by-side comparison: what does the repo do that the recipe doesn't cover?
...
```
# Agentic Cookbook Import: [Repo Name]
...
- **Existing cookbook recipe match**: [recipe name] or None
- **Proposed recipe**: `cookbook/recipes/ui/component/[name].md` or `cookbook/recipes/[category]/[name].md`
...
### [Recipe Name] v[current] → v[proposed]
...
After presenting findings, ask which items to act on. For approved items, create/update recipes and commit individually.
...
- **Bundle related things together.** A file tree browser is useless without its sync mechanism, its settings, and its git status integration. These are one recipe, not four.
- **Roadmaps are primary context.** If a Roadmap exists for a feature, it tells you WHY the feature exists, WHAT it should do, and HOW it was designed. Use this to write better recipes.
...
- **Settings are part of the component.** Every `@AppStorage`, `UserDefaults`, or config option that affects a component is part of that component's recipe. Don't split settings from the component they configure.
- **Assess infrastructure compliance.** For each bundle, check which cookbook guidelines are already met and which are missing. This becomes part of the recipe's gap analysis.
