...
domain: agentic-cookbook://recipes/ui/window/workspace-window
type: recipe
version: 1.0.0
status: accepted
language: en
...
- **REQ-001**: The window MUST use an `HSplitView` with a sidebar on the left and a detail pane on the right.
- **REQ-002**: The window MUST persist its frame (position and size) between sessions using the window frame persistence mechanism described in [window-frame-persistence.md](../window-frame-persistence.md). The autosave name MUST be derived from a hash of the workspace file path.
- **REQ-003**: The sidebar proportion MUST default to `0.3` and MUST be persisted in the workspace document's `settings` table.
- **REQ-004**: The window MUST be resizable with a minimum size sufficient to display the sidebar and detail pane without clipping content.

### Sidebar

- **REQ-005**: The sidebar MUST display two sections: "Projects" (top) and "Directories" (bottom).
- **REQ-006**: The "Projects" section MUST list all entries of type `.project`. Each row MUST display a package icon (`shippingbox.fill`, orange) and the project name, with the file path shown as secondary text.
- **REQ-007**: The "Directories" section MUST list all entries of type `.directory`. Each directory entry MUST render as a `DisclosureGroup` that expands to show auto-discovered `.catnip-proj` packages within that directory.
- **REQ-008**: Each directory entry row MUST display a folder icon (`folder.fill`) and the directory name.
- **REQ-009**: Discovered projects within a directory `DisclosureGroup` MUST display a package icon (`shippingbox.fill`, orange) and the project name.
- **REQ-010**: Double-tapping (or double-clicking) a project entry or a discovered project MUST open that project via `NSDocumentController.shared.openDocument(withContentsOf:display:)`.
- **REQ-011**: A sync progress indicator MUST be displayed at the bottom of the sidebar when any directory coordinator is syncing. Display MUST follow the pattern in [directory-sync.md](directory-sync.md) (aggregated `isSyncing` state).

### Context Menus

- **REQ-012**: Each project entry MUST have a context menu with the following items:
  - "Open Project" — opens the project (same behavior as double-tap, REQ-010)
  - "Remove from Workspace" — removes the entry from the workspace document
- **REQ-013**: Each directory entry MUST have a context menu with the following item:
  - "Remove from Workspace" — removes the entry and its associated coordinator from the workspace

### Detail Pane

- **REQ-014**: When an entry is selected in the sidebar, the detail pane MUST display an entry detail view showing metadata or project information for the selected entry.
- **REQ-015**: When no entry is selected, the detail pane MUST display an empty-state/welcome view as described in [empty-state.md](../empty-state.md), with the following action buttons:
...
- **REQ-016**: The workspace MUST be stored as a `.catnip-workspace` package (directory) containing a `workspace.db` SQLite database.
- **REQ-017**: The SQLite database MUST contain the following tables:
...
- **REQ-018**: Adding or removing an entry MUST update the workspace document, which MUST trigger `syncEntries` to reconcile the `WorkspaceDirectoryManager` coordinator pool.

### Workspace Directory Manager

- **REQ-019**: The `WorkspaceDirectoryManager` MUST manage a pool of `DirectoryWatchCoordinator` instances, one per directory entry, as specified in [directory-sync.md](directory-sync.md) REQ-027 through REQ-030.
- **REQ-020**: The manager MUST aggregate `isSyncing` across all coordinators. The workspace-level `isSyncing` MUST be `true` if any coordinator is syncing.
- **REQ-021**: The manager MUST auto-discover `.catnip-proj` packages within each watched directory and report them via an `onDiscoveryChanged` callback for document persistence.
- **REQ-022**: Each coordinator MUST use a dedicated cache directory named `cache-{entryID}` within the workspace package.

### Entry Types and Validation

- **REQ-023**: Entry type MUST be one of: `.project` (direct reference to a `.catnip-proj` file) or `.directory` (a directory scanned for projects).
- **REQ-024**: If an entry has type `.project` but its path does not end with `.catnip-proj`, the type MUST be automatically corrected to `.directory` (entry type migration).
- **REQ-025**: The workspace MUST prevent adding its own `.catnip-workspace` package as an entry (self-referential loop detection). If the user attempts to add a path that resolves to the workspace's own package, the add operation MUST be rejected and a warning MUST be logged.
- **REQ-026**: The workspace MUST prevent adding duplicate entries. If the user attempts to add a path already present as an entry, the add operation MUST be rejected.
...
| State | Behavior |
|-------|----------|
| No entries | Sidebar shows empty sections, detail pane shows welcome empty state with add buttons (REQ-015) |
| Entries present, none selected | Sidebar lists entries, detail pane shows welcome empty state (REQ-015) |
| Entry selected | Sidebar highlights selection, detail pane shows entry detail (REQ-014) |
| Directory entry expanded | DisclosureGroup open, discovered projects listed (REQ-007) |
| Directory entry collapsed | DisclosureGroup closed, discovered projects hidden |
| Syncing | SyncProgressBar visible at sidebar bottom (REQ-011), `isSyncing` true |
...
| Self-referential add rejected | Add operation silently rejected, warning logged (REQ-025) |
| Entry type migrated | Entry with incorrect type auto-corrected on load (REQ-024) |

## Accessibility

- **REQ-027**: The sidebar MUST be navigable via keyboard — arrow keys to move between entries, Return/Space to select, Right arrow to expand disclosure groups, Left arrow to collapse.
- **REQ-028**: Each project entry row MUST have an accessibility label that includes the project name and "project" role.
- **REQ-029**: Each directory entry row MUST have an accessibility label that includes the directory name and "directory" role.
- **REQ-030**: Discovered project rows MUST have accessibility labels that include the project name and "discovered project" role.
- **REQ-031**: The sync progress indicator MUST be announced by VoiceOver when its visibility changes (e.g., "Syncing directories" when it appears, "Sync complete" when it disappears).
- **REQ-032**: Context menu items MUST have descriptive accessibility labels matching their visible text.
- **REQ-033**: Tab key MUST move focus between the sidebar and detail pane.
- **REQ-034**: The empty-state action buttons MUST be accessible per [empty-state.md](../empty-state.md) REQ-009 through REQ-011.

## Conformance Test Vectors

| ID | Requirements | Input | Expected |
|----|-------------|-------|----------|
| ws-001 | REQ-001 | Open a workspace window | HSplitView renders with sidebar (left) and detail pane (right) |
| ws-002 | REQ-002 | Open workspace, move window to (100, 200), close, reopen | Window appears at (100, 200) |
| ws-003 | REQ-003 | Open workspace, do not adjust sidebar | Sidebar occupies approximately 30% of window width |
| ws-004 | REQ-003 | Adjust sidebar proportion to 0.4, close workspace, reopen | Sidebar proportion restored to 0.4 |
| ws-005 | REQ-005 | Workspace has 2 project entries and 1 directory entry | Sidebar shows "Projects" section with 2 rows and "Directories" section with 1 row |
| ws-006 | REQ-006 | Project entry named "MyApp" at `/path/to/MyApp.catnip-proj` | Row shows orange package icon, "MyApp", and path as secondary text |
| ws-007 | REQ-007 | Directory entry containing 2 `.catnip-proj` packages | DisclosureGroup expands to show 2 discovered project rows |
| ws-008 | REQ-010 | Double-click a project entry | Project opens via NSDocumentController |
| ws-009 | REQ-010 | Double-click a discovered project within a directory group | Project opens via NSDocumentController |
| ws-010 | REQ-011 | 1 of 2 directory coordinators is syncing | SyncProgressBar visible at sidebar bottom |
| ws-011 | REQ-011 | All coordinators finish syncing | SyncProgressBar hidden |
| ws-012 | REQ-012 | Right-click a project entry | Context menu shows "Open Project" and "Remove from Workspace" |
| ws-013 | REQ-013 | Right-click a directory entry | Context menu shows "Remove from Workspace" |
| ws-014 | REQ-014 | Select a project entry in sidebar | Detail pane shows project metadata/info |
| ws-015 | REQ-015 | No entry selected | Detail pane shows empty state with "Add Directory" and "Add Project" buttons |
| ws-016 | REQ-015 | Click "Add Directory" in empty state | Directory picker opens |
| ws-017 | REQ-015 | Click "Add Project" in empty state | File picker opens, filtered to .catnip-proj |
| ws-018 | REQ-016, REQ-017 | Inspect workspace package on disk | `.catnip-workspace` directory contains `workspace.db` with tables: workspace, entries, discovered_projects, settings |
| ws-019 | REQ-018 | Add a directory entry via UI | Entry appears in `entries` table, `syncEntries` fires, new coordinator created |
| ws-020 | REQ-018 | Remove a directory entry via context menu | Entry removed from `entries` table, coordinator stopped and removed |
| ws-021 | REQ-019 | Workspace with 3 directory entries | WorkspaceDirectoryManager has 3 coordinators |
| ws-022 | REQ-020 | 1 of 3 coordinators syncing | Workspace-level `isSyncing` is `true` |
| ws-023 | REQ-020 | All 3 coordinators idle | Workspace-level `isSyncing` is `false` |
| ws-024 | REQ-021 | Directory entry contains a new `.catnip-proj` package | `onDiscoveryChanged` fires, `discovered_projects` table updated |
| ws-025 | REQ-022 | Workspace with entry ID "abc" | Cache directory is `cache-abc` within workspace package |
| ws-026 | REQ-024 | Entry has type `.project` but path is `/Users/me/Code` (no `.catnip-proj` suffix) | Type auto-corrected to `.directory` |
| ws-027 | REQ-025 | Attempt to add workspace's own `.catnip-workspace` path as an entry | Add rejected, warning logged |
| ws-028 | REQ-026 | Attempt to add `/Users/me/Code` when it already exists as an entry | Add rejected |
| ws-029 | REQ-027 | Focus sidebar, press Down arrow | Selection moves to next entry |
| ws-030 | REQ-027 | Focus on collapsed directory entry, press Right arrow | DisclosureGroup expands |
| ws-031 | REQ-033 | Press Tab from sidebar | Focus moves to detail pane |
...
- **Self-referential add**: REQ-025 prevents it. The check MUST resolve symlinks and normalize paths before comparison.
- **Duplicate path add**: REQ-026 prevents it. Paths MUST be compared after normalization (resolve symlinks, remove trailing slashes).
...
- **Entry type migration on load**: If the workspace database contains entries with incorrect types (REQ-024), migration MUST happen silently on load without user intervention.
