...
| Phase 1/2/3 | The Make It Work / Make It Right / Make It Fast phases from CG-2.2 |
...
- **REQ-010**: Claude Code MUST refactor for clarity — apply separation of concerns (CG-2.13), clean up naming, ensure the code is readable and maintainable.
...
|-------|----------------|-------|
| Phase 1 | CG-1.1 (native controls), CG-1.4 (no blocking main thread), CG-2.4 (DI), CG-2.5 (immutability) | Core engineering during implementation |
| Phase 1 | CG-1.6 (unit testing), CG-14.4 (test patterns), CG-14.9 (no flaky tests), CG-14.10 (test data) | Tests written alongside code |
| Phase 1 | All opted-in items from WF-2 | Logging, accessibility, deep linking, feature flags, etc. |
| Phase 2 | CG-2.6 (fail fast), CG-2.13 (separation of concerns), CG-2.10 (explicit over implicit) | Refactoring and error handling |
| Phase 3 | CG-2.12 (tight feedback loops) | Performance measurement and optimization |
...
- **External service unavailable**: If the implementation depends on an external service that is unavailable during development, use a fake/stub (CG-14.7) and note this in the PR.
...
**Rationale**: Writing tests after all implementation is done creates a temptation to skip or rush them. Writing tests alongside code catches bugs earlier, serves as design feedback (hard-to-test code = bad design), and ensures no code ships untested. Aligns with CG-14.11 (testing workflow).
**Approved**: pending

**Decision**: Phase 3 (Make It Fast) is conditional on evidence, not automatic.
**Rationale**: Premature optimization wastes time and harms readability (CG-2.2). Most code is fast enough without optimization. Phase 3 only activates when measurement proves a problem exists.
**Approved**: pending
