# Spec Section Authoring Guide

Brief guidance on what each of the 17 spec sections should contain, common mistakes, and what makes a good vs bad entry.

## 1. Frontmatter

```yaml
---
version: 1.0.0            # Start at 1.0.0 for new specs
status: draft              # New specs start as draft
created: YYYY-MM-DD        # Today's date
last-updated: YYYY-MM-DD   # Same as created for new specs
author: claude-code        # Or the user's name
copyright: YYYY Mike Fullerton / Temporal
platforms: [...]           # Only list platforms this spec targets
tags: [...]                # 2-5 freeform labels for discoverability
dependencies: []           # Other specs this one uses, with version pins
---
```

**Common mistake**: Listing all platforms when the spec only applies to desktop (e.g., window-frame-persistence is macOS only).
...
URL patterns per platform. Skip for components that aren't navigable views.
...
Recommend native controls (Rule 1). Mention specific APIs, frameworks, and patterns.

## 17. Design Decisions

Record every subjective choice made during spec creation:
- Icon choices
