...
The most common mistake is extracting individual files as separate specs when they form a single feature bundle. **Bundle related things together.**
...
| Wrong (separate specs) | Right (one bundled spec) |
|------------------------|--------------------------|
...
### How to reference between specs

Use the `dependencies` frontmatter field:
```yaml
dependencies:
  - ui/status-bar.md@1.0.0
  - ui/Recipes/directory-sync.md@1.0.0
```

And reference in the Overview: "This recipe uses the [status-bar](../status-bar.md) component for sync progress indication."
